...
```mermaid
flowchart TD
  start["요구사항<br/>Pod A 유지 + consumer 분리 + shm 실시간 read"]
  sidecar["Sidecar container<br/>같은 Pod 내부 container 추가"]
  transport["Pod 간 전달 계층<br/>gRPC / NATS / Kafka / Redis / PVC"]
  proc["procfs 접근 검토<br/>/proc/&lt;PID&gt;/root/dev/shm"]
  direct["직접 hostPath mount<br/>/proc/&lt;PID&gt;/root/dev/shm -> Pod B"]
  privileged["privileged Pod B<br/>host /proc read-only mount"]
  final["현재 선택<br/>/host/proc/&lt;PID&gt;/root/dev/shm runtime read"]

  start --> sidecar
  sidecar -- "Pod A spec 변경 필요<br/>리소스 분리 목적과 충돌" --> transport
  transport -- "Pod A 변경 또는 별도 인프라 필요" --> proc
  proc --> direct
  direct -- "runc invalid argument" --> privileged
  privileged --> final
```
...
flowchart LR
  subgraph PodA["Pod A"]
    ...
    Volume["memory emptyDir 또는 /dev/shm"]
    Sidecar["consumer sidecar container"]
    Writer --> Volume --> Sidecar
  end
```
...
flowchart LR
  subgraph Node["Kubernetes Node"]
    ...
    Proc["host /proc<br/>/proc/&lt;PID&gt;/root/dev/shm"]

    subgraph PodB["Pod B privileged"]
      Mount["/host/proc<br/>read-only mount"]
      Finder["Pod A PID 탐색"]
      Consumer["consumer read"]
      Mount --> Finder --> Consumer
    end
